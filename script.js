...
function change_theme() {
 ...
 let item_objects = document.querySelectorAll('.merch-item-name')
 let item_names = ['Тарілка', 'Кружка', 'Ракета', 'Марсохід']
 
 for (let i = 0; i < 4; i += 1) {
   item_objects[i].innerHTML = item_names[i]
 }
 
 let price_objects = document.querySelectorAll('.merch-item-price')
 let item_price = ['1000', '1000', '2500', '5000']
 
 for (let i = 0; i < 4; i += 1) {
   price_objects[i].innerHTML = item_price[i]
 }
 
 let descr_objects = document.querySelectorAll('.merch-item-descr')
 ...
 for (let i = 0; i < 4; i += 1) {
   descr_objects[i].innerHTML = item_descr[i]
 }